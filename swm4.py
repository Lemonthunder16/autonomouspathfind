...
# =================== SAFE, INERT Q-LEARNING ===================
class SafeQL:
    """
    Minimal off-policy Q-learning that *does not* change actions.
    It passively learns from moves your current code already makes.
    Flip `apply_q_policy=True` later if you want Q to tie-break choices.
    """
    # Behavior toggles
    enabled = True
    apply_q_policy = False     # stays False: no influence on action selection
    td_clip = 1e-3             # cap TD update magnitude (keeps learning tiny)

    # Hyperparams (very conservative)
    alpha = 1e-4               # tiny learning rate
    gamma = 0.99
    epsilon = 0.0              # no exploration (inert)
    ...
    def encode_state(pos):
        # State = position only (keeps things stable & simple)
        return (int(pos[0]), int(pos[1]))

    def observe(self, s, a, r, s2, done=False):
        """Passive Q-learning update from a transition (s,a,r,s2)."""
        ...
        td = target - qsa
        # clip tiny updates to avoid any drift
        if td > SafeQL.td_clip:
            td = SafeQL.td_clip
        elif td < -SafeQL.td_clip:
            td = -SafeQL.td_clip
        self.Q[s][a] = qsa + SafeQL.alpha * td
        self.episode_reward += r

    def pick_best(self, s, candidates):
        """
        Optional tie-breaker using Q-values among candidate next cells.
        Kept inert unless apply_q_policy=True.
        """
        if not SafeQL.apply_q_policy or not candidates:
            return candidates[0] if candidates else None
        # choose candidate with highest Q from current state toward that cell
...
class Robot:
    def __init__(self, name, color, start, grid, shared_map):
        ...
        # minimal Q-learning learner (inert for control)
    ...
    def _do_move(self, newpos):
        ...
        # Perform the move (no control change)
        self.pos = newpos
    ...
    def move_step(self):
        ...
        # Greedy pick neighbor closer to goal from frontier (unchanged behavior)
        next_cell = None
        ...
        if next_cell:
            ...
            # (Optional future tie-break): next_cell = self.ql.pick_best(self.pos, [next_cell, ...])
...
def main():
    ...
    pygame.display.set_caption("Swarm Robots Exploration - Robust Victory Tracing (with inert Q-learning)")
    ...
    while running:
        ...
        for r in robots:
            ...
            rx, ry = r.pos
            pygame.draw.circle(screen, r.color, (rx*CELL_SIZE, ry*CELL_SIZE), CELL_SIZE//2)
